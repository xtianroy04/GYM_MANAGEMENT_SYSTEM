...
<nav aria-label="breadcrumb" class="d-flex justify-content-end me-3 mb-4">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('reportsPayments')}}">Payments</a></li>
                            ...
                            @forelse ($payments as $payment)
                                <tr>
                                    ...
                                    @if($payment->payment_for == 'Annual-Fee')
                                        <td>{{ date('F j, Y g:i A', strtotime($payment->member->membership->annual_date_started)) }}</td>
                                    @else
                                        <td>{{ date('F j, Y g:i A', strtotime($payment->member->membership->date_started)) }}</td>
                                    @endif
                                    @if($payment->payment_for == 'Annual-Fee')
                                        <td>{{ date('F j, Y g:i A', strtotime($payment->member->membership->annual_date_end)) }}</td>
                                    @else
                                        <td>{{ date('F j, Y g:i A', strtotime($payment->member->membership->date_end)) }}</td>
                                    @endif
                                </tr>
                            @empty
                                <tr>
